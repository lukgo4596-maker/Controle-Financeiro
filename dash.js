...
function getTotalToReceive() {
  ...
  dividas.forEach(d => {
    if (d.status === "pendente") {
      ...
      const parcelasRestantes = d.parcelas - parcelasPagas;
      const valorRestante = d.valorParcela * parcelasRestantes;
      total += valorRestante;
    }
  });
  return total;
...
function getNextDueInfo() {
  ...
  const parcelasPagas = closestDebt.parcelasPagas || 0;
  const parcelasRestantes = closestDebt.parcelas - parcelasPagas;
  const valorRestante = closestDebt.valorParcela * parcelasRestantes;
...
function renderPendingList() {
  ...
  recentDebts.forEach(debt => {
    ...
    const parcelasRestantes = debt.parcelas - parcelasPagas;
    const valorRestante = debt.valorParcela * parcelasRestantes;
    const parcelasInfo = `${parcelasPagas}/${debt.parcelas} pagas - ${parcelasRestantes} restantes`;
    ...
    debtEl.innerHTML = `
      <div class="debt-info">
        ...
        <div class="debt-parcelas">${parcelasInfo} - ${formatCurrency(debt.valorParcela)}/parcela</div>
      </div>
    `;
